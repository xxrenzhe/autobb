...
20.优化“一键上广告”的功能
- 第一步：生成广告创意并评分，解释生成的广告创意和评分依据
- 用户可以点击“重新生成”来生成最多3个广告创意，并进行对比分析
- 用户选择最满意的一个广告创意，然后进入下一步
- 广告创意的生成依赖之前网页抓取和AI分析的数据
- 广告创意需要包含：headline/description/keyword/callout/sitelink等配置广告系列所需的核心信息
- 每个广告创意只专注一个主题，避免主题混杂
- 广告创意的生成也是优先使用Vertex AI，其次使用Gemini API，若都没有配置，则文案提醒用户去/settings页面配置，同时中止流程
- 第二步：配置广告系列/广告组/广告所需要的参数，然后生成一组完整的广告系列/广告组/广告
- Final URL需要配置在广告层级，而Final URL suffix则需要配置在广告系列层级
- 第三步：完成Offer与Ads账号的关联，完成并验证Ads账号授权，确保可以发布广告系列/广告组/广告
- 第四步：汇总待发布的信息，用户点击“发布广告”后完成广告发布上线
- 给用户提供一个“是否暂停所有已存在的广告系列”的勾选项，若用户勾选，则在广告发布前先暂停所有已存在的广告系列
- 后续异步操作：Ads账号的状态检测、广告系列/广告组/广告的表现数据都需要归属到已关联的Offer，广告系列/广告组/广告的表现数据为AI广告创意生成提供真实的投放数据等
注意：
- 所有页面和按钮都使用中文，具体参数使用“中文（英文）”的方式
- 通过context7 mcp调研，找到通过Google Ads API正确绑定Ads账号、配置广告系列/广告组/广告的正确方式
- 根据新的“一键上广告”的优化流程，同步优化登录后左边栏导航中的“广告系列”、“创意管理”、“投放评分”、“Google Ads账号”、“数据管理”等的功能和展示方式

21.评估功能完整性和用户体验：快速创建Offer、快速关联Offer和Google Ads账号，快速为Offer创建广告、快速生成广告创意、真实上线高质量广告、广告表现数据每日同步、广告表现数据大盘展现

22.增加offer投放评分功能，当用户添加一个offer后，点击操作栏的“投放分析”按钮，弹窗显示分析过程和最终评分，通过网站数据抓取、品牌词搜索量查询等功能获取相关数据，再通过AI构建一套评分系统，判断这个offer投放google ads的ROI，一定要使用真实的数据维度，且这些数据存储在相应Offer下，便于后续广告文案生成复用
...
24.梳理现有的广告生成/发布/数据反馈的功能，需要有一套符合KISS原则的不断迭代优化的机制，包括AI创意优化/关键词优化/预算优化/广告系列筛选等，不断提高广告的CTR/降低CPC/提升ROI
- 是否能够快速测试Offer的投放效果？
...
25.通过context7 mcp获得Google Ads API的详细文档，获得最新最真实的访问接口，确保对于Google Ads的操作真实有效，并引入一些有利于提高广告的CTR/降低CPC/提升ROI的新功能（保持KISS原则，不需要太复杂的功能）

26.支持“批量导入Offer”功能，支持csv文件格式，并在前端提供模版文件下载

27.在数据大盘增加“风险提示”板块，并实现：
...
29.基于你对项目的深刻理解，帮我补充/细化/完善/优化产品定位、产品目标、对外宣传的特点等，最终生成一个对外宣传的营销页面作为首页
...
30.在首页营销页面（域名是www.autoads.dev）增加一个“立即开始”的按钮，引导至登录页面，登录后进入系统（域名是app.autoads.dev）
- 只有首页营销页面可以免登录访问，在未登录状态下访问其他页面都会被强制跳转到登录页面
- 普通用户登录后，可以访问并使用全部的业务功能和页面
- 管理员和普通用户，登录后访问的是同一套前端UI/UX界面
- 管理员登录后，除了可以访问并使用全部的业务功能和页面外，还可以访问管理员专属的页面，比如“用户管理”、“数据库备份历史”等

31.用户手动创建Offer或通过文件上传批量创建Offer时，可以填写产品的价格（product_price）和佣金比例（commission_payout）这2个参数，这2个是可选参数，示例如下：
...
32.创建一个网页数据抓取和AI创意生成的测试页面（/admin/scrape-test），放在管理员功能区，实现如下功能：
...
33.优化网站的SEO信息，突出品牌特征，相关图片参考public目录

34.全面梳理UI/UX设计，确保UI界面美观简洁，符合现代化顶级SaaS产品的风格；优化UX设计，确保用户体验流畅高效，比如适合用弹窗的场景就不需要使用单独的页面

35.根据 docs/RequirementsV1.md 和 .env 中的提供的信息，启动本地服务（强制3000端口），并在本地开发环境完成所有功能的真实测试，登录账号使用autoads，不要使用模拟功能或模拟数据，若缺少测试所需的参数，请及时提出来，我们沟通后确定，千万不要自作主张